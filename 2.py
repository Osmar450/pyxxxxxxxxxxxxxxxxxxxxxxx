...
def show_game_over(elapsed_time):
    ...
    time_text = menu_font.render("Time Survived: " + format_time(elapsed_time), True, RED)  # Mostrar tiempo transcurrido
    ...
    screen.blit(level_text, ((WIDTH - level_text.get_width()) // 2, HEIGHT // 2 + 60))  # Mostrar nivel debajo de las monedas
...
def reiniciar_juego():
    ...
    enemies.clear()
    coins.clear()
    enemies.extend([Enemy(random.randint(50, WIDTH - enemy_image.get_width() - 50), random.randint(50, HEIGHT - enemy_image.get_height() - 50)) for _ in range(10)])
...
enemies = [Enemy(random.randint(50, WIDTH - enemy_image.get_width() - 50), random.randint(50, HEIGHT - enemy_image.get_height() - 50)) for _ in range(10)]
...
# Dibujar el fondo
    ...
    if not game_over and not player.paused:
        ...
        if player.coins >= 5 * player.level:
            ...
            enemies.clear()
            enemies.extend([Enemy(random.randint(50, WIDTH - enemy_image.get_width() - 50), random.randint(50, HEIGHT - enemy_image.get_height() - 50)) for _ in range(player.level * 2)])
